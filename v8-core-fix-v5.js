...
function bindTrackingLabels(scene){
  ...
  state.labelsBound = labels.length > 0;
}
  ...
  pistonBanks.forEach(bank=>{
    ...
    for(let idx=0;idx<4;idx++){
      ...
      state.pistonUnits.push({
        sign,idx,z:[-2.25,-0.75,0.75,2.25][idx],piston,pin,rod,cap,
...
function updateMainScene(){
  ...
  state.rpm=lerp(state.rpm,target,1-Math.exp(-dt*1.2));
  if(state.rpm>0.01) state.cycle=(state.cycle+state.rpm/60*360*dt)%720;
...
function PatchedRenderer(...args){
  ...
  renderer.render=function(scene,camera){
    try{
      ...
      console.warn('[V8 core fix v5]',err);
    }
